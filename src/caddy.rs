...
use crate::{config::Config, store::App};

pub fn render_block(apps: &[App]) -> String {
    let mut out = String::new();
    out.push_str("# This file is managed by hostingctl. Do not edit manually.\n\n");
    for app in apps {
        out.push_str(&format!(
            r#"# client: {client} app: {slug}
{domain} {{
  encode zstd gzip
...
pub fn apply(cfg: &Config, apps: &[App], reload: bool) -> Result<()> {
    let path = &cfg.caddy_managed_path;
    let previous = fs::read_to_string(path).ok();
    let next = render_block(apps);
